...
Here is a function which can detect if a given list of sentences is an instance of *Disjunction Elimination*.  

```{python}
def disjunction_elimination(prop_list):
  if len(prop_list) != 3: return False
  # For disjunction elimination, one proposition must be a disjunction
  # and the other must be a negation.
  disj, neg = prop_list[0], prop_list[1]
  if type(disj) != Disjunction: 
    disj, neg = prop_list[1], prop_list[0]
  ...
  if under != disj.left:
    other = disj.left
    if under != disj.right: return False
  return other == prop_list[2]
```
