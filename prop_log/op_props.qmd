...
In order to help us capture this idea, we will let $\Gamma$ represent any set of propositions.^[So $\Gamma$ could be the empty set or the entire theory of Euclidean geometry.  It is just any set of propositions.]  We then say that $\Gamma$ **entails** a proposition $\alpha$ if every variable evaluation which satisfies $\Gamma$ also satisfies $\alpha$.
...
Write a Python function which takes two arguments -- the first argument is a list (or set) of propostions, and the second argument is a single proposition.  The function then returns `True` if the set of propositions entails the second argument, and `False` otherwise.
