...
- If $a|n$ then $\frac n a$ is a natural number.
...
- If $\frac n a$ is a natural number, then $a|n$.
...
- If $r \ge d$ then *r* is not a lower bound of *S*.
...
(6. Showing $x=q_2d+r_2$ and $0\le r_2<d$ implies $q=q_2$ and $r=r_2$.)
...
(1. $L$ is nonempty bounded below integers)
