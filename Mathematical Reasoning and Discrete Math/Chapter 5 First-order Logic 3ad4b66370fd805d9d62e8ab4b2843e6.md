...
Let *U* be the domain of discourse and $\mathcal M$ a model.

We assign $(\forall x P(x))^{\mathcal M}=T$ if for every choice of $u\in U$ we have $u\in P^{\mathcal M}$.  Otherwise $(\forall xP(x))^{\mathcal M}=F$.

We assign $(\exists xP(x))^{\mathcal M} = T$ if there is some choice of $u\in U$ such that $u\in P^{\mathcal M}$.  Otherwise $(\exists xP(x))^{\mathcal M} = F$.
...
Then $\forall x R(x)^{\mathcal M}=F$ because not all of the objects in the domain are red.  

However $\exists xR(x)^{\mathcal M}=T$ because some object in the domain is red.  

***Exercise***

Let $U = \Bbb N$.  Let $P(x)$ be the predicate “*x* is positive”, and $Q(x)$ is the predicate “*x* is negative”, and $R(x)$ the predicate “*x* is equal to 1”.
...
Pick any object, like say, the red cube.  Let’s call it *u*.  Now let’s evaluate $(K(u)\to W(u))^{\mathcal M}$.  By the semantics of the conditional, this is $(K(u))^{\mathcal M} \leadsto (W(u))^{\mathcal M}$.  Because *u* is not black, $K(u)^{\mathcal M}=F$.  Because *u* is not white, $W(u)^{\mathcal M}=F$.  Therefore 

$$
\begin{aligned}
 (K(u)\to W(u))^{\mathcal M} &= K(u)^{\mathcal M}\leadsto W(u)^{\mathcal M} \\
&= F\leadsto F \\
&= T
\end{aligned}
$$
...
Now let *u* be the white cylinder.  Evaluate $(K(u)\to W(u))^{\mathcal M}$.

Next, explain why $(\forall x (K(x)\to W(x))^{\mathcal M} = T$. 

 ***Exercise***

Let’s consider a property, *P,* and a model, $\mathcal M$, such that $P(u)^{\mathcal M} = T$ for every choice of *u* in the domain.  

Certain it follows that $\forall x P(x)^{\mathcal M}=T$.

Now prove that $\forall x(P(x)\lor Q(x))^{\mathcal M}=T$.

Also prove that $(\forall x P(x)\lor \forall x Q(x))^{\mathcal M}=T$.

***Exercise***

Consider a property, *P*, and model, $\mathcal M$, such that $(P(u)\lor Q(u))^{\mathcal M} = T$ for every *u* in the domain.

It follows immediately by definition that $\forall x(P(x)\lor Q(x))^{\mathcal M}=T$.
...
Notice that this set is defined by the “is even” property.  If we use the symbol *E* for the “is even” property, then the following proposition is true (in a model with universe $\mathbb N$).
...
To evaluate $(\forall x\forall y(N(x,y)\to L(x,y)))^{\mathcal M}$ formally, we need to consider five total possible assignments to *x*.  
...
- $x\mapsto a$
With this assignment we now have to evaluate $(\forall y (N(a,y)\to L(a,y)))^{\mathcal M}$.  To do this we again need to consider five possible assignments to *y*.
    - $y\mapsto a$
    With this assignment we now have to evaluate $(N(a,a)\to L(a,a))^{\mathcal M}$.  Noting that $N(a,a)^{\mathcal M}=F$ and $L(a,a)^{\mathcal M}=T$, then
        
        $$
        \begin{aligned}
         (N(a,a)\to L(a,a))^{\mathcal M} &= N(a,a)^{\mathcal M} \leadsto L(a,a)^{\mathcal M} \\
         &= F\leadsto T \\
         &= T
        \end{aligned}
        $$
    ...
    With this assignment
        ...
        $$
        \begin{aligned}
         (N(a,b)\to L(a,b))^{\mathcal M} &= N(a,b)^{\mathcal M} \leadsto L(a,b)^{\mathcal M} \\
         &= T\leadsto T \\
         &= T
        \end{aligned}
        $$
    ...
    - $y\mapsto c$
        ...
        $$
        \begin{aligned}
         (N(a,c)\to L(a,c))^{\mathcal M} &= N(a,c)^{\mathcal M} \leadsto L(a,c)^{\mathcal M} \\
         &= F\leadsto T \\
         &= T
        \end{aligned}
        $$
    ...
    - $y\mapsto d$
        ...
        $$
        \begin{aligned}
         (N(a,d)\to L(a,d))^{\mathcal M} &= N(a,d)^{\mathcal M} \leadsto L(a,d)^{\mathcal M} \\
         &= F\leadsto F \\
         &= T
        \end{aligned}
        $$
    ...
    $$
    \begin{aligned}
     (N(a,e)\to L(a,e))^{\mathcal M} &= N(a,e)^{\mathcal M} \leadsto L(a,e)^{\mathcal M} \\
     &= F\leadsto F \\
     &= T
    \end{aligned}
    $$
    ...
    Therefore $(\forall y(N(a,y)\to L(a,y)))^{\mathcal M} = T$.
    
- $x\mapsto b$
    
    ***Exercise***
    
     Perform this assignment and evaluate $(\forall y(N(b,y)\to L(b,y)))^{\mathcal M}$.
...
- $x\mapsto d$
    - $y\mapsto a$
        ...
        $$
        \begin{aligned}
         (N(d,a)\to L(d,a))^{\mathcal M} &= N(d,a)^{\mathcal M} \leadsto L(d,a)^{\mathcal M} \\
         &= F\leadsto F \\
         &= T
        \end{aligned}
        $$
    ...
    With this assignment
        ...
        $$
        \begin{aligned}
         (N(d,b)\to L(a,b))^{\mathcal M} &= N(d,b)^{\mathcal M} \leadsto L(d,b)^{\mathcal M} \\
         &= F\leadsto T \\
         &= T
        \end{aligned}
        $$
    ...
    - $y\mapsto c$
        ...
        $$
        \begin{aligned}
         (N(d,c)\to L(d,c))^{\mathcal M} &= N(d,c)^{\mathcal M} \leadsto L(d,c)^{\mathcal M} \\
         &= F\leadsto T \\
         &= T
        \end{aligned}
        $$
    ...
    - $y\mapsto d$
        ...
        $$
        \begin{aligned}
         (N(d,d)\to L(d,d))^{\mathcal M} &= N(d,d)^{\mathcal M} \leadsto L(d,d)^{\mathcal M} \\
         &= F\leadsto F \\
         &= T
        \end{aligned}
        $$
    ...
    $$
    \begin{aligned}
     (N(d,e)\to L(d,e))^{\mathcal M} &= N(d,e)^{\mathcal M} \leadsto L(d,e)^{\mathcal M} \\
     &= T\leadsto T \\
     &= T
    \end{aligned}
    $$
...
- $x\mapsto e$
We should check this case too, but I promise $(\forall y(N(e,y)\to L(e,y)))^{\mathcal M}=T$.  However, you are invited to check for yourself if you would like more exercise.

The above now confirms that, for every possible assignment to *x*, the resulting proposition is true.  

Therefore it demonstrates $(\forall x\forall y (N(x,y)\to L(x,y)))^{\mathcal M}$.
...
But this is more effort than you really need to do.  When it comes to an existential quantifier, you really just need to exhibit *one* instance of *T*, not *every* instance of *T*.  So, for each assignment of *x*, if you find one satisfying assignment of *T*, you can stop early!
...
Let $\mathcal U$ be any nonempty set, called the **universe**.

Let $\mathcal I$ be a function such that, for each $a\in \text{Obj}$, the expression $a^{\mathcal I}$ is the **interpretation of *a***, which denotes the element of $\mathcal U$ to which *a* is mapped.  

$$
a^{\mathcal I} \in \mathcal U
$$

Moreover let $f\in \text{Funcs}$ and $n=\text{Arity}(f)$.  The expression $f^{\mathcal I}$ is the **interpretation of *f***, which denotes the function to which *f* is mapped.

$$
f^{\mathcal I}:\mathcal U^n\to\mathcal U
$$

Moreover, let $P\in \text{Preds}$ and $n=\text{Arity}$.  The expression $P^{\mathcal I}$ is the **interpretation of *P***, which denotes the subset of $\mathcal U^n$ to which *P* is mapped.  

$$
P^{\mathcal I} \subseteq \mathcal U^n
$$

Let $\mathcal M = (\mathcal U,\mathcal I)$.  

Let $v: \text{Vars}\to \mathcal U$ be a function, which we call a **variable assignment**.  
...
$$
\overline v(f(t_1,...,t_n)) = f^{\mathcal I}(\overline v(t_1),...,\overline v(t_n))
$$

For a formula $\phi\in\text{Forms}$, model $\mathcal M$, and variable assignment *v*, we will define what it means for **the model and assignment to satisfy the formula**, denoted 

$$
\mathcal M,v\vDash \phi
$$

We use $\mathcal M,v\not\vDash\phi$ to express that $\mathcal M, v\vDash \phi$ does not hold.

- If $P\in\text{Preds}$ and $n=\text{Arity}$ and $t_1,…,t_n\in\text{Terms}$, and if we have
    
    $$
    (\overline v(t_1),...,\overline v(t_n))\in P^{\mathcal I}
    $$
    
    then $\mathcal M,v\vDash P(t_1,…,t_n)$.
    
- If $\phi,\psi\in\text{Form}$ then
    - If $\mathcal M,v\not\vDash \phi$ then $\mathcal M,v\vDash (\neg \phi)$.
    - If $\mathcal M,v\vDash \phi$ and $\mathcal M,v\vDash \psi$ then $\mathcal M,v\vDash (\phi\land\psi)$.
    - If $\mathcal M,v\vDash \phi$ or $\mathcal M,v\vDash \psi$ then $\mathcal M,v\vDash (\phi\lor\psi)$.
    - If $\mathcal M,v\not\vDash \phi$ or $\mathcal M,v\vDash \psi$ then $\mathcal M,v\vDash (\phi\to\psi)$.
    - If $\mathcal M,v\vDash \phi \to \psi$ and $\mathcal M,v\vDash \psi\to\phi$ then $\mathcal M,v\vDash (\phi\leftrightarrow\psi)$.
    - Suppose that $x\in \text{Vars}$, and for every $u\in\mathcal U$ we have $\mathcal M,v[x\mapsto u]\vDash \phi$.  Then $\mathcal M,v\vDash (\forall x\phi)$.
    - Suppose that $x\in \text{Vars}$ and for some $u\in\mathcal U$ we have $\mathcal M,v[x\mapsto u]\vDash \phi$.  Then $\mathcal M,v\vDash (\exists x\phi)$.

Finally we can define **truth in the model $\mathcal M$**.  

Let $\phi$ be a closed formula.  Then we say that $\phi$ is **true in the model $\mathcal M$**, and write $\mathcal M\vDash \phi$, if for every variable assignment *v* we have $\mathcal M,v\vDash \phi$.  
