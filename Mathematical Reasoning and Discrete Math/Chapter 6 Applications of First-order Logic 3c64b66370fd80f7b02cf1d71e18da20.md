...
For example, if $\mathcal M$ is a model with universe $\{a,b,c\}$, then to directly confirm the truth of $\forall x P(x)$, you have to confirm $P(x)$ for three different assignments of *x*.  

However, that *disconfirming* $\forall x P(x)$ can (in principle) take much less labor.  To show that $\forall x P(x)^{\mathcal M}=F$ we only need to find *one* domain element $u\in\{a,b,c\}$, for which $P(u)^{\mathcal M}=F$.
...
Consider a model $\mathcal M$ with domain $\mathcal U = \{a,b,c,d,e\}$.
...
Show that $\forall x P(x)^{\mathcal M} = F$ by exhibiting a single element of the domain, $u\in\mathcal U$, for which $u\notin P^{\mathcal M}$.
...
Suppose that we have a model in which $P(x)$ is false for every element of the domain.  That is to say, if *U* is the domain and $u\in U$, then $P(u)^{\mathcal M}=F$.

Let’s now evaluate 

$$
\forall x (P(x)\to Q(x))^{\mathcal M}
$$
...
$$
\begin{aligned}
 (P(u)\to Q(u))^{\mathcal M} &= P(u)^{\mathcal M}\leadsto Q(u)^{\mathcal M} \\
 &= F \leadsto Q(u)^{\mathcal M} \\
 &= T
\end{aligned}
$$

Notice this last equality!  Even without knowing the value of $Q(u)^{\mathcal M}$, we can still judge that $F\leadsto Q(u)^{\mathcal M} = T$.  

That is because of how the boolean conditional $\leadsto$ is defined.  Whenever the antecedent is false, $\leadsto$ returns true.  So whether $Q(u)^{\mathcal M}=T$ or $Q(u)^{\mathcal M}=F$, either way the result will be $F\leadsto Q(u)^{\mathcal M}=T$.

We have just shown that $(P(u)\to Q(u))^{\mathcal M}=T$ for every $u\in U$, which shows that 

$$
\forall x(P(x)\to Q(x))^{\mathcal M}=T
$$
...
for every choice of model $\mathcal M$.
...
How can we demonstrate that it is a tautology?  Let $\mathcal M$ be a model and *u* any element of the domain.  We need to evaluate 

$$
(P(u)\lor\neg P(u))^{\mathcal M} = P(u)\curlyvee \sim P(u)^{\mathcal M}
$$

If $P(u)^{\mathcal M}=T$ then we compute 

$$
\begin{aligned}
T\ \curlyvee \sim T &= T\curlyvee F \\
&= T
\end{aligned}
$$

On the other hand if $P(u)^{\mathcal M}=F$ then 

$$
\begin{aligned}
 F\curlyvee\sim F &= F\curlyvee T \\
&= T
\end{aligned}
$$

This demonstrates that $\forall x(P(x)\lor\neg P(x))^{\mathcal M}=T$ no matter what we choose for $\mathcal M$.  
...
Let *R* be a relation, and $\mathcal M$ a model.  
...
Not reflexive because $R(b,b)^{\mathcal M}=F$, although other counter-examples could be given too.
...
Not symmetric because $(R(a,b)\to R(b,a))^{\mathcal M}=F$.

For example, “*x* and *y* have the same absolute value” is symmetric.  That is to say, the relation *R* is defined by $R(x,y)^{\mathcal M}=T$ if and only if 
...
Not transitive because $((R(a,d)\land R(d,c))\to R(a,c))^{\mathcal M}=F$. One other counter-example is possible.
...
Consider the random example of a relation *R* and model $\mathcal M$ with domain $\mathcal U = \{a,b,c\}$.  If we have 
...
Below, we will look at every possible assignment of domain elements to the variables, and judge whether $\forall x\forall y((E(x)\land E(y))\to x=y)^{\mathcal M}=T$.
...
- $x\mapsto 1,y\mapsto 1$
    ...
    $$
    \begin{aligned}
     ((E(1)\land E(1))\to 1=1)^{\mathcal M} &= (E(1)\land E(1))^{\mathcal M} \leadsto (1=1)^{\mathcal M}\\
     &= (E(1)^{\mathcal M} \curlywedge E(1)^{\mathcal M})\leadsto T \\
     &= (F\curlywedge F) \leadsto T \\
     &= F\leadsto T \\
     &= T
    \end{aligned}
    $$
...
- $x\mapsto 1,y\mapsto 2$.
    ...
    $$
    \begin{aligned}
     ((E(1)\land E(2))\to 1=2)^{\mathcal M} &= (E(1)\land E(2))^{\mathcal M} \leadsto (1=2)^{\mathcal M}\\
     &= (E(1)^{\mathcal M} \curlywedge E(2)^{\mathcal M})\leadsto F \\
     &= (F\curlywedge T) \leadsto F \\
     &= F\leadsto T \\
     &= T
    \end{aligned}
    $$
...
$$
\begin{aligned}
 ((E(2)\land E(2))\to 2=2)^{\mathcal M} &= (E(2)\land E(2))^{\mathcal M} \leadsto (2=2)^{\mathcal M}\\
 &= (E(2)^{\mathcal M} \curlywedge E(2)^{\mathcal M})\leadsto T \\
 &= (T\curlywedge T) \leadsto T \\
 &= T\leadsto T \\
 &= T
\end{aligned}
$$
...
Pick one more possible assignment and evaluate $((E(x)\land E(y))\to x=y)^{\mathcal M}$ using that assignment.  

Explain why $(\forall x\forall y ((E(x)\land E(y))\to x=y))^{\mathcal M}=T$.
...
$$
\begin{aligned}
 ((O(1)\land O(3))\to 1=3)^{\mathcal M} &= (O(1)\land O(3))^{\mathcal M} \leadsto (1=3)^{\mathcal M}\\
 &= (O(1)^{\mathcal M} \curlywedge O(3)^{\mathcal M})\leadsto F \\
 &= (T\curlywedge T) \leadsto F \\
 &= T\leadsto F \\
 &= F
\end{aligned}
$$
...
This is because, technically speaking, for both existential quantifiers we can select the same element.  If we select *u* for both of them, where $P(u)^{\mathcal M}=T$, then the proposition is true.
