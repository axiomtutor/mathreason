...
We will often use lower-case italic Latin letters as symbols for objects, like $a,b,…,z$.  If we need more symbols we will use indexed symbols, like $a_1,a_2,\dots,b_1,b_2,\dots,z_1,z_2,...$ as well.  

We will use upper-case italic Latin letters as symbols for predicates, also allowing for indices.  So the predicate symbols are $A, A_1, A_2,…,B,B_1,B_2,…,Z,Z_1,Z_2,…$
...
Then we call $P(a_1,…,a_n)$ an **atomic formula**.
...
Let $P\in \text{Preds}$ and $n=\text{Arity}(P)$, and $a_1,…,a_n\in \mathcal U$.  

Then 

$$
(P(a_1,...,a_n))^{\mathcal M} = T \ \text{ \ \ if } (a_1^{\mathcal I},...,a_n^{\mathcal I})\in P^{\mathcal I}
$$

and 

$$
(P(a_1,...,a_n))^{\mathcal M} = F \ \text{ \ \ if } (a_1^{\mathcal I},...,a_n^{\mathcal I})\notin P^{\mathcal I}
$$

- What is the difference between a superscript $\mathcal M$ and a superscript $\mathcal I$?
...
Since 1 is not in the set of even numbers, $o^{\mathcal M}\notin P^{\mathcal M}$, and therefore $(P(o))^{\mathcal M}=F$.  This is exactly the result that we intuitively know that we should obtain.  
...
This is everything that we need to now evaluate $(R(b))^{\mathcal M}$.  Because $b^{\mathcal M}$ is a red ball in the room, it therefore is a red object in the room, and therefore $b^{\mathcal M}\in R^{\mathcal M}$.  So it follows, by definition, that $(R(b))^{\mathcal M}=T$.
...
Let’s then determine $((P(a)\land \neg Q(a))\to P(b))^{\mathcal M}$.  It should be clear that $(P(a))^{\mathcal M} = T$ and $(Q(a))^{\mathcal M} = T$, and $(P(b))^{\mathcal M} = F$.

$$
\begin{aligned}
 ((P(a)\land \neg Q(a))\to P(b))^{\mathcal M} &= (P(a)\land \neg Q(a))^{\mathcal M} \leadsto (P(b))^{\mathcal M} \\
 &= ((P(a))^{\mathcal M}\curlywedge (\neg Q(a))^{\mathcal M}) \leadsto F \\
 &= (T\curlywedge \sim (Q(a))^{\mathcal M})\leadsto F \\
 &= (T\curlywedge \sim T)\leadsto F \\
 &= (T\curlywedge F)\leadsto F\\
 &= F\leadsto F\\
 &= T
\end{aligned}
$$
...
If $P\in \text{Preds}$ and $n = \text{Arity}(P)$, and $a_1,…,a_n\in\text{Objs}$,, then 
...
- If $\phi\in\text{Atom}$ and $\phi=P(a_1,…,a_n)$ for some $P\in\text{Preds}$ and $n = \text{Arity}(P)$ and $a_1,…,a_n\in\text{Objs}$, then
    
    $$
    \phi^{\mathcal M} = T \text{ \ \ if } (a_1^{\mathcal I},...,a_n^{\mathcal I})\in P^{\mathcal I}
    $$
    
    and 
    
    $$
    \phi^{\mathcal M} = F \text{ \ \ if } (a_1^{\mathcal I},...,a_n^{\mathcal I})\notin P^{\mathcal I}
    $$
