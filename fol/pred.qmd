...
We will call all constant, predicate, and relation sybmols **nonlogical symbols**.^[This does not mean that the symbols are in any way illogical.  It just means that they do not have their truth-value determined *merely* by logic.]
...
Consider a language for natural numbers.  For each natural number $n$, our language will have a constant symbol $c_n$ which we can use to refer to it.  Also it will have a relation $L$ to express that one number is less than another.  